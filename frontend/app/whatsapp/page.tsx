...
import { api, WhatsAppLog } from "@/lib/api";
...
export default function WhatsAppPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.listWhatsApp(currentOrg.organization_id).then(setLogs).finally(() => setLoading(false));
  }, [currentOrg]);
  ...
  return (
    <div>
      ...
      <p className={styles.hint}>
        Aucun message n'est réellement délivré en mode Mock (compte WhatsApp Business API réel à connecter
        plus tard) — cette page prouve que l'envoi a bien été déclenché, avec le bon contenu.
      </p>
      ...
      {loading ? (
        ...
        <div className="surface-card">
          <div className={styles.emptyState}>
            ...
            <p>Aucun message envoyé pour l'instant. Ils apparaissent ici dès qu'un prospect intéressé reçoit une offre.</p>
          </div>
        </div>
      ) : (
        logs.map((log) => (
